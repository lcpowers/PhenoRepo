...
team_ID = "CU_Pheno" 
theme="phenology"
forecast_file=paste0("~/Desktop/CurrentProjects/TeamRotation/PhenoRepo/data/forecasts/",theme,"-",Sys.Date(),"-",team_ID,".csv")
```
...
# Restructure error df from wide to long 
...
ggplot(error_melt,aes(x=forecast_day,y=deviation))+
...
ggplot(subset(MSQerror_df,forecast_day<=35), aes(x=forecast_day,y=MSQerror))+
  geom_point(alpha=0.5)+
  geom_smooth()
...
plot(forecast_df$forecast_day,forecast_df$gcc_sd)
```
...
ggplot(subset(all_gcc,time>="2019-01-01"), aes(x=time,y=gcc_90))+
  geom_point(aes(color=source),alpha=0.75)+
  geom_ribbon(data = subset(all_gcc,time>="2021-05-01"),
              mapping = aes(x=time,ymin=gcc_90-gcc_sd,ymax=gcc_90+gcc_sd,color=source,fill=source),alpha=0.75)+
  facet_wrap(~siteID)+
  theme_classic(base_size = 16)+
  scale_color_manual(values=c("grey60","springgreen4"),labels=c("Observed","Forecasted"))+
  scale_fill_manual(values=c("grey60","springgreen4"),labels=c("Observed","Forecasted"))+
...
metadata_yaml <- paste0("~/Desktop/CurrentProjects/TeamRotation/PhenoRepo/data/forecasts/",theme,"-",Sys.Date(),"-",team_ID,".yml")

create_model_metadata(forecast_fn)
