...
## To do's:
# Figure out why parameter dataframe isn't working
# Troubleshoot uncertainty
...
for(site in sites){
  ...
  params_df$green_up = optim_fit$par["green_up"]
  params_df$G_max = optim_fit$par["G_max"]
  ...
  ggplot() +
    ...
    labs(x="Day of year",y="GCC 90",title="Model Fit") +
...
# Dataframe to store fitted parameters for each site
cv_params_df <- NULL
error_df <- data.frame(day=seq(yday(paste0('2021',spring_date)),yday(paste0('2021',fall_date))))
...
for(site in sites){
  ...
  for(yeari in years){
    ...
    error_df <- merge(error_df,tmp_error,by='day',all=TRUE)
