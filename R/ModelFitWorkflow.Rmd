...
for(site in sites){
  ...
  ###########################################################################
  ####    Hardcoded this -- fix later  -------------------------------
  # Remove 2017 for GRSM - that year is whack
  if (site == "GRSM"){
  ...
  gridsearch_fit <- gridsearch(pvecs, ssq_phenmod, y=site_targs$gcc_90, GDD=site_GDD)
  
  ####### DELETE EVENTUALLY: NLL fit attempts
  # # First pass at parameter fitting using gridsearch and NLL
  # pvecs$sd <- mean(targs$gcc_sd,na.rm=T)
  # gridsearch_fit <- gridsearch(pvecs, nll_phenmod, y=site_targs$gcc_90, GDD = site_GDD, targets=site_targs)
  ####### DELETE EVENTUALLY: NLL fit attempts
  ...
  optim_fit <- optim(optim_starts,ssq_phenmod,y=site_targs$gcc_90, GDD = site_GDD)
  ...
  #                   gridsearch_fit$par["t1"],
  #                   gridsearch_fit$par["t2"],
  #                   gridsearch_fit$par["sd"])
  # optim_fit <- optim(optim_starts,nll_phenmod,y=site_targs$gcc_90,GDD=site_GDD, targets=site_targs)
  ...
  model_results <-  WarmModel(site_GDD,G_init,a,b,t1,t2)
  ...
  ggplot() +
    ...
    labs(x="Day of year",y="GCC 90", title="1/gdd_days_passed") +
