...
pvecs <- list(G_init=seq(min(targets$gcc_90)*0.9,max(targets$gcc_90)*1.1,length.out=10), # initial GCC
              ...
              K = seq(max(targets$gcc_90)*1.1,1,length.out=10))  # Summer transition
...
```{r}
p = pvecs <- list(G_init=targets$gcc_90[1], # initial GCC
              a=0.005,   # green-up: fast growth
              b=0.0005,  # maturation
              t1=mean(temp$daily_diff),   # Spring transition
              t2=mean(temp$daily_diff),
              K = 1) 
```
