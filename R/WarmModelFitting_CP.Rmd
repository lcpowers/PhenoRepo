...
optim_fit <- optim(starts, ssq_phenmod, y=targets$gcc_90, temp_df=temp, spring_date, fall_date)
optim_fit
save.image(file = paste0("R/optimized_WarmModel_CP_",Sys.Date(),".RData"),list=c(gridsearch_fit,optim_fit)) # Save data frame 
```
...
starts <- c(G_init=G_init,a,b,c,t1,t2,K,sd=sd_start)

fit_phenomod <- optim(par=starts, # Starting parameter values
                      nll_phenmod, # Function used
                      temp_df=temp,spring_date = spring_date,fall_date = fall_date) # Other inputs
fit_phenomod
```
