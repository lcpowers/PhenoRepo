...
source("R/gridsearch_Casey.R") #for gridsearch() function
source("R/WarmModel_josh.R")
...
ssq_phenmod <- function(p,y,GDD) {
  ...
  #is self documenting
  y_pred <- WarmModel_josh(GDD,G_init=p[1],a=p[2],b=p[3],c=p[4],t1=p[5],t2=p[6]) #predicted y
...
fit
save.image(paste0("R/optimized_WarmModel_josh_",Sys.Date(),".RData")) # Save data frame 
...
model_results <-  WarmModel_josh(GDD,G_init,a,b,c,t1,t2)
