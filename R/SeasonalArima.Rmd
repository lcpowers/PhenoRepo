...
for(s in 1:length(sites)){
  ...
  tmp_4cast_df <- data.frame(time = seq(Sys.Date(), by="day",length.out = 35),
                            siteID = sites[s],
...
save(list=ls(all.names=T),file="R/ARIMA_4132021.Rdata")
```
...
scores <- neon4cast::score(example,theme=c("phenology"))
```
